...
adwr_spatial = x %>% 
  filter(source == 'AZ') %>% 
    select(dtw, date, date_min, date_max, measurement_dist, source, wellid)
...
r0 =  dtw_range(x, -200, 0) %>%  select(dtw, date, date_min, date_max, measurement_dist, source, wellid)
...
leaflet() %>%
  ...
  addCircleMarkers(data = adwr_spatial,
                   ...
                   weight = .5,
                   stroke = T,
                   popup = leafpop::popupTable(st_drop_geometry(adwr_spatial), feature.id = FALSE,
 ...
 addCircleMarkers(data = r0, #clusterOptions = markerClusterOptions(interactive()),
                  ...
                  color = 'black',
                   ...
                   weight = .5,
                   stroke = TRUE,
                   popup = leafpop::popupTable(st_drop_geometry(r0), feature.id = FALSE,
...
# **Range in average well depth**

***

> The following plots display wells that have at least 40 years of records and also have at least 100 ft difference between there minumum and maximum depth measurements
...
az_plotly = ggplotly(az_gg, tooltip = c('x', 'y', 'wellid')) %>%
...
usgs_range = join_time %>% 
  filter(source == 'USGS', range >= 100, measurement_dist >= 20, time_span >= 40)

usgs_gg = ggplot(data = usgs_range) +
  geom_pointrange(
    mapping = aes(x = wellid, y = dtw),
    stat = "summary",
    fun.min = min,
...
usgs_plotly = ggplotly(usgs_gg, tooltip = c('x', 'y', 'wellid')) %>%
  style(hoverlabel = label) %>% 
  layout(font = font, 
         yaxis = list(fixedrange = TRUE))
usgs_plotly
...
cases[2] <- cell_spec(cases[[2]], color = "black", bold = T)
    # background = spec_color(1:4, end = 0.9, option = "D", direction = 1))

kbl(cases, col.names = c('Case', 'Count'), escape = F, align = "c") %>%
