...
# Sites Lat, long
az_site <- readNWISdata(stateCd="Arizona", #for Arizona
...
#Join
az_nwis_join <- left_join(az_nwis_dtw, az_site, by = "site_no") %>% #join site information to each dtw site row 
...
az_shp_4269 <- us_states(resolution = "high", states = "AZ") %>%  #read in AZ shp file
  st_transform(crs = 4269) #set CRS for NAD83

st_crs(az_shp_4269) #was 4326, after st_transform is 4269 #NEEDED TO REPROJECT ALL .SHP (do not have to manually assign crs because crs is defined)
...
az_clip <- st_intersection(az_shp_4269, az_nwis_spatial) %>% # This filters for points inside of the az shape file
  select(site_id)
